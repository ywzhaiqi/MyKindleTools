...
class MobiHeader:
    ...
    def get_exth(self, ext=''):
        ...
        for _ in range(num_items):
            ...
            if id in MobiHeader.id_map_strings.keys():
                exth_name = MobiHeader.id_map_strings[id]
                outArr.append('{0: >3d} {1: <30s} {2:s}'.format(id, exth_name, unicode(content, codec).encode("utf-8")))
            elif id in MobiHeader.id_map_values.keys():
                exth_name = MobiHeader.id_map_values[id]
                if size == 9:
                    value, = struct.unpack('B',content)
                    outArr.append('{0:3d} {1:<30s} {2:d}'.format(id, exth_name, value))
                elif size == 10:
                    value, = struct.unpack('>H',content)
                    outArr.append('{0:3d} {1:<30s} 0x{2:0>4X} ({2:d})'.format(id, exth_name, value))
                elif size == 12:
                    value, = struct.unpack('>L',content)
                    outArr.append('{0:3d} {1:<30s} 0x{2:0>8X} ({2:d})'.format(id, exth_name, value))
                else:
                    outArr.append('{0: >3d} {1: <30s} (0x{2:s})'.format(id, "Bad size for "+exth_name, content.encode('hex')))
            elif id in MobiHeader.id_map_hexstrings.keys():
                exth_name = MobiHeader.id_map_hexstrings[id]
                outArr.append('{0:3d} {1:<30s} 0x{2:s}'.format(id, exth_name, content.encode('hex')))
            else:
                exth_name = "Unknown EXTH ID {0:d}".format(id)
                outArr.append("{0: >3d} {1: <30s} 0x{2:s}".format(id, exth_name, content.encode('hex')))
            pos += size
