...
#  0.77   fix bug when unpacking HDImages is used in conjunction with added Fonts
...
def processMobi8(mh, metadata, sect, files, rscnames, pagemapproc, k8resc, obfuscate_data, apnxfile=None, epubver='2'):
    ...
    # create the opf
    opf = OPFProcessor(files, metadata.copy(), fileinfo, rscnames, True, mh, usedmap, pagemapxml, guidetext, k8resc, epubver)
...
def processMobi7(mh, metadata, sect, files, rscnames):
    ...
    # create an OPF
    opf = OPFProcessor(files, metadata, fileinfo, rscnames, ncx.isNCX, mh, usedmap, pagemapxml, guidetext)
    opf.writeOPF()
