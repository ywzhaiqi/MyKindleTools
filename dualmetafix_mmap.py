...
def getASIN(azw3_path):
    f = open(pathof(azw3_path),"r+b")
    datain = mmap.mmap(f.fileno(),0)
    datain_rec0 = readsection(datain,0)

    rec0 = datain_rec0
    asins = read_exth(rec0, 113)
    if len(asins):
        asin = asins[0]
        return asin
...
class DualMobiMetaFix:
    ...
    def __init__(self, infile, outfile, asin):
        if infile != outfile:
            shutil.copyfile(pathof(infile), pathof(outfile))
        f = open(pathof(outfile),"r+b")
        ...
        # 如果 asin 是 azw3 路径，则提取其中的 asin
        if os.path.isfile(pathof(asin)):
            asin = getASIN(asin)

        # 如果没有 ASIN 则人造 ASIN
        if asin == '':
            asin = str(uuid.uuid4())
            asin = asin.replace('-', '').upper()
            # 生成封面缩略图
...
def main(argv=utf8_argv()):
    ...
    try:
        ...
        dmf = DualMobiMetaFix(infile, outfile, asin)
        dmf.getresult()
